...
void GaussianBlurNode::set_render_target(const std::shared_ptr<SingleRenderTarget>& renderTarget_) {
	if (renderTarget_) {
		...
		renderTarget = temp;
		// 半分の大きさにさせる
		temp->initialize(WinApp::GetClientWidth() / 2, WinApp::GetClientHight() / 2, DXGI_FORMAT_R8G8B8A8_UNORM_SRGB);
...
void GaussianBlurNode::create_pipeline_state() {
	...
	shaderManager.initialize(
		...
		"Resources/GameScene/HLSL/PostEffetct/GaussianBlur.PS.hlsl"
	);
